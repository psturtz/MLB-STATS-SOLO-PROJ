...
const LoginBall = (props) => {
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute top-10 left-[8.5rem]">LOGIN</div>
      <div className="absolute w-32 top-36 left-8">
        <form onSubmit={handleSubmit}>
          <input
            className={
              formValidity.usernameEntered
                ? 'absolute left-0 w-28 border border-sky-500'
                : 'absolute left-0 w-28 border border-sky-500 placeholder:text-rose-600'
            }
            name="username"
            type="text"
            placeholder={
              formValidity.usernameEntered ? 'Username' : 'Enter a username'
            }
          ...
          <input
            className={
              formValidity.passwordEntered
                ? 'absolute left-36 w-28 border border-sky-500'
                : 'absolute left-36 w-28 border border-sky-500 placeholder:text-rose-600'
            }
            type="password"
            placeholder={
              formValidity.passwordEntered ? 'Password' : 'Enter a password'
            }
            onChange={(e) => setPassword(e.target.value)}
            value={password}
          ></input>
          <p className={formValidity.correctPassword ? "hidden" : "absolute top-6 left-0 w-64 text-sm text-rose-600"}>Username and/or Password is incorrect</p>

          <button
            type="submit"
            className="absolute top-[5.75rem] left-24 w-16 border border-sky-500"
          >
            Log In
          </button>
        </form>
        <Link
          to={'/register/'}
          className="absolute top-[7.5rem] left-12 w-40 text-xs text-center underline :hover text-red-500"
        >
